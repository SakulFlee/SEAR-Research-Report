...
We hypothesise that \textit{dependency-injection} in java is possible and that with enabling the use of dynamic and optional dependencies, the risk of potential threads increases drastically.

In summary, the report concludes that \textit{dependency-injection} is indeed possible and, without further securing an application, can pose a high risk not only to the application, but also the system and user.
However, there are also benefits, such as being able to extend an application to the users' needs dynamically, that may or may not are worth taking.
